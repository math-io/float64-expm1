...
*       |                              |
*
*     expm1(r) = exp(r)-1 is then computed by the following specific way which minimize the accumulation rounding error:
*
*                          2     3
*                         r     r    [ 3 - (R1 + R1*r/2)  ]
*         expm1(r) = r + --- + --- * [--------------------]
...
function expm1( x ) {
	...
	if ( hx >= 0x4043687A ) {			/* if |x|>=56*ln2 */
		if ( hx >= 0x40862E42 ) {		/* if |x|>=709.78... */
			if ( hx >= 0x7ff00000 ) {
				if ( ( (hx&0xfffff) | lowWord(x) ) !== 0 ) {
					...
					return ( xsb === false ) ? x : -1.0;/* exp(+-inf)={inf,-1} */
				}
			}
			if ( x > O_THRESHOLD ) {
				return PINF;
		...
		if ( xsb === true ) { /* x < -56*ln2, return -1.0 with inexact */
